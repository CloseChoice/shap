...
class InputGateModel(tf.keras.Model):
    def __init__(self):
        ...
        self.fc_ii = tf.keras.layers.Dense(1, use_bias=True, activation=None)
        self.fc_hi = tf.keras.layers.Dense(1, use_bias=True, activation=None)
        self.inputs = None
        self.outputs = None
...
_ = model((x, h))
weights_ii = np.array([[1., 1., 0.]], dtype=np.float32)
bias_ii = np.array([0.2], dtype=np.float32)
weights_hi = np.array([[2., 1., 1.]], dtype=np.float32)
bias_hi = np.array([0.32], dtype=np.float32)
...
shap_values = exp.shap_values([x, h])
